...
class SplashScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        top_spacer = tk.Frame(self, bg=BG)
        ...
        enter_btn = tk.Button(btn_frame, text="Enter", command=lambda: open_customer_menu_in_root(self.controller.container), **style_btn)
        enter_btn.grid(row=0, column=0, padx=18, pady=10)

        emp_btn = tk.Button(btn_frame, text="Employee Login", command=lambda: self.controller.show_frame("login_screen"), **style_btn)
        emp_btn.grid(row=0, column=1, padx=18, pady=10)

        # Bottom spacer
        bottom_spacer = tk.Frame(self, bg=BG)
        bottom_spacer.pack(fill="both", expand=True)
        
        # small footer note
        foot = tk.Label(self, text="For staff access use 'Employee Login'", bg=BG, fg=FG, font=("Arial", 9))
...
def open_customer_menu_in_root(container):
    """Show the CustomerMenuFrame inside the container."""
    if CustomerMenuFrame is None:
        messagebox.showerror("Missing module", f"Could not import CustomerMenuFrame")
        return

    # This is called from the splash screen which is part of the main app
    # We need to get the controller from the root window
    root = container.winfo_toplevel()
    if hasattr(root, 'show_frame'):
        root.show_frame("customer_menu")
